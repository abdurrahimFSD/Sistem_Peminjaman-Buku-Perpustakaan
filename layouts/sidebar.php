        <!-- Sidebar Start -->
        <aside class="left-sidebar with-vertical">
            <div>
                <!-- ---------------------------------- -->
                <!-- Start Vertical Layout Sidebar -->
                <!-- ---------------------------------- -->

                <!-- Start Logo -->
                <div class="brand-logo d-flex align-items-center justify-content-between">
                    <a href="#" class="text-nowrap logo-img">
                        <h3 class="mb-0">
                            <iconify-icon icon="tdesign:system-code" class="align-middle ms-2 me-1"></iconify-icon> <span class="align-middle">APP</span>
                        </h3>
                    </a>
                    <a href="javascript:void(0)" class="sidebartoggler ms-auto mt-2 text-decoration-none fs-5 d-block d-xl-none">
                        <iconify-icon icon="tabler:x"></iconify-icon>
                    </a>
                </div>
                <!-- End Logo -->
                
                <!-- Start Sidebar Items -->
                <nav class="sidebar-nav scroll-sidebar" data-simplebar>
                    <ul id="sidebarnav">
                        <!-- ---------------------------------- -->
                        <!-- Start Home -->
                        <!-- ---------------------------------- -->
                        <li class="nav-small-cap">
                            <iconify-icon icon="tabler:dots" class="nav-small-cap-icon fs-4"></iconify-icon>
                            <!-- <span class="nav-small-cap-icon fs-1">Home</span> -->
                            <span class="hide-menu">Home</span>
                        </li>
                        <li class="sidebar-item">
                            <a class="sidebar-link" href="?page=home" id="get-url" aria-expanded="false">
                                <span>
                                    <iconify-icon icon="tabler:device-analytics"></iconify-icon>
                                </span>
                                <span class="hide-menu">Dashboard</span>
                            </a>
                        </li>
                        <!-- ---------------------------------- -->
                        <!-- End Home -->
                        <!-- ---------------------------------- -->
                        
                        <!-- ---------------------------------- -->
                        <!-- Start Master Data -->
                        <!-- ---------------------------------- -->
                        <li class="nav-small-cap">
                            <iconify-icon icon="tabler:dots" class="nav-small-cap-icon fs-4"></iconify-icon>
                            <span class="hide-menu">Master Data</span>
                        </li>
                        <li class="sidebar-item">
                            <a class="sidebar-link has-arrow" href="javascript:void(0)" aria-expanded="false">
                                <span>
                                    <iconify-icon icon="tabler:book-2"></iconify-icon>
                                </span>
                                <span class="hide-menu">Buku</span>
                            </a>
                            <ul aria-expanded="false" class="collapse first-level">
                                <li class="sidebar-item">
                                    <a href="?page=bukuData" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Data Buku</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="?page=bukuCreate" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Tambah Data</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="#" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Edit Data</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="#" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Detail Data</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <li class="sidebar-item">
                            <a class="sidebar-link has-arrow" href="javascript:void(0)" aria-expanded="false">
                                <span class="d-flex">
                                    <iconify-icon icon="tabler:users"></iconify-icon>
                                </span>
                                <span class="hide-menu">Anggota</span>
                            </a>
                            <ul aria-expanded="false" class="collapse first-level">
                                <li class="sidebar-item">
                                    <a href="?page=anggotaData" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Data Anggota</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="?page=anggotaCreate" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Tambah Data</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="#" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Edit Data</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                        <!-- ---------------------------------- -->
                        <!-- End Master Data -->
                        <!-- ---------------------------------- -->
                        
                        
                        
                        <!-- ---------------------------------- -->
                        <!-- Start Transaksi -->
                        <!-- ---------------------------------- -->
                        <li class="nav-small-cap">
                            <iconify-icon icon="tabler:dots" class="nav-small-cap-icon fs-4"></iconify-icon>
                            <span class="hide-menu">Transaksi</span>
                        </li>
                        <li class="sidebar-item">
                            <a class="sidebar-link has-arrow" href="javascript:void(0)" aria-expanded="false">
                                <span class="d-flex">
                                    <iconify-icon icon="tabler:book-upload"></iconify-icon>
                                </span>
                                <span class="hide-menu">Peminjaman</span>
                            </a>
                            <ul aria-expanded="false" class="collapse first-level">
                                <li class="sidebar-item">
                                    <a href="?page=peminjamanData" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Data Peminjaman</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="?page=peminjamanCreate" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Tambah Data</span>
                                    </a>
                                </li>
                                <li class="sidebar-item">
                                    <a href="#" class="sidebar-link">
                                        <div class="round-16 d-flex align-items-center justify-content-center">
                                            <iconify-icon icon="tabler:circle"></iconify-icon>
                                        </div>
                                        <span class="hide-menu">Edit Data</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                </nav>
                <!-- End Sidebari Items -->

                <!-- ---------------------------------- -->
                <!-- End Vertical Layout Sidebar -->
                <!-- ---------------------------------- -->
            </div>
        </aside>
        <!--  Sidebar End -->